...
\begin{erinnerung} ~\
	\begin{enumerate}
		...
		\item CV: $V_1 = \dotsc = V_n = V$
			...
			wobei $v(x_1, \dotsc, x_n) = \mathbb{E}\left[ V |~X_1 = x_1, \dotsc, X_n = x_n \right]$.
	\end{enumerate}		
\end{erinnerung}
~\newline
\begin{satz}
...
\begin{enumerate}
	...
	\item Die Auktion wird als Zweitpreisauktion (SA) ohne Limitpreis durchgeführt. ~\\
		...
		\begin{proof}
			\begin{enumerate}
				\item SA: 
					$$ \beta^{SA}(x_{i}) = w(x_{i}, x_{i}) = x_{i} + \frac{1}{2} \Big( x_{i} + \mathds{E}\big[ X_{(3)} \big| \underbrace{X_{(2)} = x_{i}}_{\overset{preisbestimmendes}{Signal}} \big] \Big) $$
					Grafisch:
					\tikz[baseline=-0.5ex]{  
					 	\draw(0,0)--(8,0);
...
\begin{enumerate}
	...
	\item ~\
		...
		\end{itemize}
	\item EA, $n=4$
		$$ x_1 = 5 = x_{(3)}, ~x_2 = 6 = x_{(2)}, ~x_{(3)} = 7 = x_{(1)}, ~x_{4} = 4 = x_{(4)} $$
